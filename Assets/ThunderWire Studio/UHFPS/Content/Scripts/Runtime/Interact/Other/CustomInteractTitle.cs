...
namespace UHFPS.Runtime
{
    public class CustomInteractTitle : MonoBehaviour, IInteractTitle
    {
        ...
        private void Awake()
        {
            if (OverrideTitle && UseTitleDynamic) DynamicTitle.Initialize();
            if (OverrideUseTitle && UseUseTitleDynamic) DynamicUseTitle.Initialize();
            if (OverrideExamineTitle && UseExamineTitleDynamic) DynamicExamineTitle.Initialize();
        }
