...
namespace UHFPS.Runtime
{
    ...
    public class DraggableItem : SaveableBehaviour
    {
        ...
        [Tooltip("Should we return the object back to the original parent when we drop it?")]
        ...
        private void OnCollisionEnter(Collision collision)
        {
            ...
            if(!KeepParent) transform.SetParent(collision.transform);
        }
        ...
        {
            int index = GameTools.RandomUnique(0, ImpactSounds.Length, lastImpact);
            AudioClip audioClip = ImpactSounds[index];
