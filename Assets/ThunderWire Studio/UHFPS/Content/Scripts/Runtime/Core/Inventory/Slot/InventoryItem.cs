        ...
        */
        ...
        private void Update()
        {
            if (Inventory != null)
            {
                ...
                if (isMoving && !isRotating && mouseDelta.magnitude > 0)
                {
                    ...
                    for (int y = 0; y < Inventory.MaxSlotRows; y++)
                    {
                        for (int x = 0; x < Inventory.SlotColumns; x++)
                        {
                            ...
                            {
                                if (Inventory.IsCoordsOverflowed(x + dimensions.x, y + dimensions.y))
                                    continue;
        ...
        private bool CheckPutSpace()
        {
            if(orientation == Orientation.Vertical)
                return Inventory.CheckSpaceFromPosition(currentSlot.x, currentSlot.y, Item.Height, Item.Width, this);
            return Inventory.CheckSpaceFromPosition(currentSlot.x, currentSlot.y, Item.Width, Item.Height, this);
        }
