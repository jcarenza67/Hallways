...
namespace UHFPS.Runtime
{
    public class GameManager : Singleton<GameManager>
    {
        ...
        public TMText ControlsInfoText;
        public TMText InteractText;
        [Multiline] public string InteractFormat;
        ...
        /// </summary>
        public void ShowControlsInfo(bool show, string controlsText)
        {
            ControlsInfoPanel.SetActive(show);
            if(show) ControlsInfoText.text = controlsText;
        }

        /// <summary>
        /// Show the helper interact controls panel.
        /// </summary>
        public void ShowInteractInfo(bool show, string title = "", string button1 = "", string button2 = "")
        {
            TMText tmText = InteractText;
            tmText.gameObject.SetActive(show);
            if (!show) return;

            string text = InteractFormat;
            text = Replace(text, "[title]", title, "[button1]");
            text = Replace(text, "[button1]", button1, "[button2]");
            text = Replace(text, "[button2]", button2, "null");
            tmText.text = text;
        }

        private string Replace(string str, string tag, string replacement, string nextTag)
        {
            if (string.IsNullOrEmpty(replacement))
            {
                ...
                if (nextTag != "null")
                {
                    int nextIndex = str.IndexOf(nextTag);
                    return str.Remove(index, nextIndex - index);
                }
                return str.Remove(index, str.Length - index);
            }

            return str.Replace(tag, replacement);
        }
