...
using static UHFPS.Input.InputManager.ActionMap;

namespace UHFPS.Input
{
    public class InputManager : Singleton<InputManager>
    {
        public const string INPUTS_FILENAME = "Inputs.xml";
        ...
        public BehaviorSubject<bool> OnApply = new(false);
        ...
        {
            get => Path.Combine(SerializationUtillity.SerializationAsset.GetConfigPath(), INPUTS_FILENAME);
        }
        ...
        private async void Awake()
        {
            ...
            {
                Debug.Log(InputsPath);
                await ReadInputOverrides();
                if (debugMode) Debug.Log($"[InputManager] {INPUTS_FILENAME} readed successfully.");
            }
        ...
        public static InputAction Action(string name)
        {
            ...
            Debug.LogError(new NullReferenceException($"Could not find input action with name \"{name}\"!").ToString());
            return null;
        }
        ...
        public static ActionMap.Action ActionMapAction(string name)
        {
            ...
            Debug.LogError(new NullReferenceException($"Could not find action map action with name \"{name}\"!").ToString());
            return null;
        }
        ...
        public static void SetInputRebindOverrides()
        {
            ...
            if (Instance.debugMode) Debug.Log("Bindings Applied");
        ...
        {
            if (Instance.debugMode) Debug.Log("Bindings Discarded");
            Instance.preparedRebinds.Clear();
        ...
        public static void ResetInputsToDefaults()
        {
            ...
            foreach (var map in Instance.actionMap)
            {
                foreach (var action in map.Value.actions)
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(binding.Value.bindingPath.overridePath))
                        {
        ...
        private void PerformInteractiveRebinding(InputAction action, int bindingIndex)
        {
            rebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .OnComplete(_ =>
                {
                    if (debugMode) Debug.Log("Rebind Completed");
                    inputActions.Enable();
                ...
                .OnCancel(_ =>
                {
                    if (debugMode) Debug.Log("Rebind Cancelled");
                    inputActions.Enable();
        ...
        public sealed class BindingPath
        {
            ...
            {
                get => observer.Select(_ => glyphPath);
            }
            ...
            {
                glyphPath = SpritesAsset.GetGlyphPath(EffectivePath);
            }
            ...
            {
                return string.Format(format, glyphPath);
            }
        }
    }
