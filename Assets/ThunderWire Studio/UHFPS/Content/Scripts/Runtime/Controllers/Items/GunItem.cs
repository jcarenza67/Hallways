...
namespace UHFPS.Runtime
{
    public class GunItem : PlayerItemBehaviour
    {
        ...
        private void Awake()
        {
            ...
            defaultAim = EnableSway ? SwayPivot.localPosition : ItemObject.transform.localPosition;
        ...
        public override void OnUpdate()
        {
            ...
            {
                Transform aimTransform = EnableSway ? SwayPivot : ItemObject.transform;
                if (CanInteract && !isReloading && InputManager.ReadButton(Controls.ADS))
                {
                    aimTransform.localPosition = Vector3.Lerp(aimTransform.localPosition, aimingSettings.AimPosition, Time.deltaTime * aimingSettings.AimSpeed);
                ...
                {
                    aimTransform.localPosition = Vector3.Lerp(aimTransform.localPosition, defaultAim, Time.deltaTime * aimingSettings.BackAimSpeed);
                    if (gunProperties.EnableAdsFov)
                    {
